    ...
    , toolBar(*this)
{
    ...
    addView(new CABSLGraphViewObject("Docs.BehaviorControl2015", "BehaviorControl2015", "Options.h"),
...
void ConsoleRoboCupCtrl::executeFile(const std::string& name1,
...
void ConsoleRoboCupCtrl::help(In& stream)
{
    ...
    list("  call <file> [<file>] : Execute a script file. If the optional script file is present, execute it instead.",
         pattern,
         true);
    list("  ci off | on | <fps> : Switch the calculation of images on or off or activate it and set the frame rate.",
         pattern,
         true);
    list("  cls : Clear console window.", pattern, true);
    list("  dt off | on | <fps> : Delay time of a simulation step to real time or a certain number of frames per "
         "second.",
         pattern,
         true);
    list("  echo <text> : Print text into console window. Useful in console.con.", pattern, true);
    list("  gc initial | ready | set | playing | finished | kickOffBlue | kickOffRed | outByBlue | outByRed | "
         "gamePlayoff | gameRoundRobin : Set GameController state.",
         pattern,
         true);
    list("  help | ? [<pattern>] : Display this text.", pattern, true);
    list("  robot ? | all | <name> {<name>} : Connect console to a set of active robots. Alternatively, double click "
         "on robot.",
         pattern,
         true);
    ...
    list("  jc hide | show | motion <num> <command> | ( press | release ) <button> <command> : Set joystick motion "
         "(use $1 .. $6) or button command.",
         pattern,
         true);
    list("  jm <axis> ( off | <button> <button> ) : Map two buttons on an axis.", pattern, true);
    list("  js <axis> <speed> <threshold> [<center>] : Set axis maximum speed and ignore threshold for \"jc motion "
         "<num>\" commands.",
         pattern,
         true);
    list("  log start | stop | clear | save <file> | full | jpeg : Record log file and (de)activate image compression.",
         pattern,
         true);
    ...
    list("  log ? [<pattern>] | load <file> | ( keep | remove ) <message> {<message>} : Load, filter, and display "
         "information about log file.",
         pattern,
         true);
    list("  log start | pause | stop | forward [image] | backward [image] | repeat | goto <number> | time <minutes> "
         "<seconds> | cycle | once | fastForward | fastBackward : Replay log file.",
         pattern,
         true);
    list("  mof : Recompile motion net and send it to the robot. ", pattern, true);
    list("  msg off | on | log <file> | enable | disable : Switch output of text messages on or off. Log text messages "
         "to a file. Switch message handling on or off.",
         pattern,
         true);
    list("  mr ? [<pattern>] | modules [<pattern>] | save | <representation> ( ? [<pattern>] | <module> | off ) : Send "
         "module request.",
         pattern,
         true);
    list("  mv <x> <y> <z> [<rotx> <roty> <rotz>] : Move the selected simulated robot to the given position.",
         pattern,
         true);
    ...
    list("  pr none | illegalBallContact | playerPushing | illegalMotionInSet | inactivePlayer | illegalDefender | "
         "leavingTheField | kickOffGoal | requestForPickup | manual : Penalize robot.",
         pattern,
         true);
    list("  qfr queue | replace | reject | collect <seconds> | save [<seconds>] : Send queue fill request.",
         pattern,
         true);
    list("  set ? [<pattern>] | <key> ( ? | unchanged | <data> ) : Change debug data or show its specification.",
         pattern,
         true);
    ...
    list("  v3 ? [<pattern>] | <image> [jpeg] [lower | upper] [<name>] : Add a set of 3-D views for a certain image.",
         pattern,
         true);
    ...
    list("  vfd ? [<pattern>] | ( all | <name> ) ( ? [<pattern>] | <drawing> ( on | off ) ) : (De)activate debug "
         "drawing in field view.",
         pattern,
         true);
    list("  vi ? [<pattern>] | <image> [jpeg] [segmented] [lower | upper] [<name>] [gain <value>] : Add image view.",
         pattern,
         true);
    list("  vid ? [<pattern>] | ( all | <name> ) ( ? [<pattern>] | <drawing> ( on | off ) ) : (De)activate debug "
         "drawing in image view.",
         pattern,
         true);
    list("  vp <name> <numOfValues> <minValue> <maxValue> [<yUnit> <xUnit> <xScale>]: Add plot view.", pattern, true);
    list("  vpd ? [<pattern>] | <name> ( ? [<pattern>] | <drawing> ( ? [<pattern>] | <color> [<description>] | off ) ) "
         ": Plot data in a certain color in plot view.",
         pattern,
         true);
}
...
void ConsoleRoboCupCtrl::createCompletion()
{
    ...
    if(debugRequestTable)
	{
	    for(const auto& i : debugRequestTable->slowIndex)
		{
		    ...
		    if(i.first.substr(0, 13) == "debug images:")
			{
			    ...
			    completion.insert(std::string("vi ") + translate(i.first.substr(13)) +
			                      " jpeg segmented upper");
    ...
    if(drawingManager)
	{
	    ...
	    for(const auto& drawingsPair : drawingManager->drawings)
		{
		    if(!strcmp(drawingManager->getDrawingType(drawingsPair.first), "drawingOnImage") && imageViews)
			{
			    ...
			    for(const auto& imageViewPair : *imageViews)
				{
				    completion.insert(std::string("vid ") + imageViewPair.first + " " +
				                      translate(drawingsPair.first) + " on");
				    completion.insert(std::string("vid ") + imageViewPair.first + " " +
				                      translate(drawingsPair.first) + " off");
				}
			}
		    else if(!strcmp(drawingManager->getDrawingType(drawingsPair.first), "drawingOnField") && fieldViews)
			{
				...
				{
				    completion.insert(std::string("vfd ") + fieldViewPair.first + " " +
				                      translate(drawingsPair.first) + " on");
				    completion.insert(std::string("vfd ") + fieldViewPair.first + " " +
				                      translate(drawingsPair.first) + " off");
				}
	...
	for(const auto& plotPair : *plotViews)
		...
		if(translate(i.first).substr(0, 5) == "plot:")
			...
			completion.insert(std::string("vpd ") + plotPair.first + " " + translate(i.first).substr(5) +
			                  " off");
		    }
...
void ConsoleRoboCupCtrl::completeConsoleCommand(std::string& command, bool forward, bool nextSection)
{
    ...
    if(currentCompletionIndex == completion.end() ||
       std::strncmp((*currentCompletionIndex).c_str(), command.c_str(), command.length()))
	return;
    ...
    if(forward)
	{
	    if(!nextSection)
		{
		    ...
		    while(currentCompletionIndex != completion.end() &&
		          lastCompletion == handleCompletionString(command.length(), *currentCompletionIndex))
			++currentCompletionIndex;
    ...
    else
	{
	    if(!nextSection)
		{
		    ...
		    while(currentCompletionIndex != completion.begin() &&
		          lastCompletion == handleCompletionString(command.length(), *currentCompletionIndex))
			--currentCompletionIndex;
