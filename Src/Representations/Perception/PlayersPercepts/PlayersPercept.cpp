...
void PlayersPercept::draw() const
{
  DEBUG_DRAWING("representation:PlayersPercept:image", "drawingOnImage")
  {
    for(const auto& player : players)
    {
      ...
      if (player.detectedJersey && player.ownTeam)
          DRAWTEXT("representation:PlayersPercept:image", player.x1, player.y1, 20, ColorRGBA::yellow,"TEAMMATE");
      else
    	  DRAWTEXT("representation:PlayersPercept:image", player.x1, player.y1, 20, ColorRGBA::cyan,"OTHERS");
