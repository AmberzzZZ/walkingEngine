...
RoleCompressed::RoleCompressed(const Role& role_) :
  role(role_.role),
  wasPenalized(role_.wasPenalized),
  requestType(role_.requestType),
  isRequestNotAllowed(role_.isRequestNotAllowed),
  previewR(role_.previewR)
{}

RoleCompressed::operator Role() const
{
  Role role;
  role.role = this->role;
  role.wasPenalized = wasPenalized;
  role.isRequestNotAllowed = isRequestNotAllowed;
  role.previewR = previewR;
  return role;
}
