...
void ParticleSwarm::start()
{
  ...
  for(unsigned int i = 0; i < particles.size(); ++i)
  {
    ...
    for(unsigned int j = 0; j < dimensions.size(); ++j)
    {
      ...
      float random = i == 0 ? 0.f : Random::bernoulli(); // the first particles consists of the given start values
...
void ParticleSwarm::updateParticle(Particle& particle)
{
  ...
  for(unsigned int i = 0; i < particle.position.size(); ++i)
  {
    particle.velocity[i] = velocityFactor * particle.velocity[i]
      + bestPositionFactor * Random::bernoulli() * (particle.bestPosition[i] - particle.position[i])
      + globalBestPositionFactor * Random::bernoulli() * (bestParticle.bestPosition[i] - particle.position[i])
      + randomPositionFactor * Random::bernoulli() * ((dimensions[i].min + Random::bernoulli() * (dimensions[i].max - dimensions[i].min)) - particle.position[i]);
