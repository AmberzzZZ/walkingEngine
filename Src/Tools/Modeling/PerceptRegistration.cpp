...
PerceptRegistration::PerceptRegistration(const CameraMatrix& cameraMatrix,
	...
	const GoalPercept2017& GoalPercept2017,
  ...
  theFrameInfo(frameInfo), theGoalFeature(goalFeature), theGoalFrame(goalFrame), theGoalPercept2017(GoalPercept2017), theFieldLineIntersections(fieldLineIntersections),
...
int PerceptRegistration::registerLandmarks(std::vector<RegisteredLandmark>& landmarks)
{
  ...
  if(theGoalPercept2017.goalPosts.size() > 0)
  {
      std::vector<GoalPercept2017::GoalPost> goalPost = theGoalPercept2017.goalPosts;
      for(int i = 0; i < goalPost.size(); i++)
      {
    	  Vector2f PostPosition = goalPost[i].locationOnField;
    	  Vector2f postInWorld;
    	  if(getAssociatedUnknownGoalPost(PostPosition, postInWorld))
    	  {
    		  RegisteredLandmark newLandmark;
			  newLandmark.w = postInWorld;
			  newLandmark.p = PostPosition;
			  newLandmark.cov = UKFPose2D::getCovOfPointInWorld(newLandmark.p, 0.f, theCameraMatrix, inverseCameraMatrix, currentRotationDeviation);
			  landmarks.push_back(newLandmark);
    	  }
      }
      numOfLandmarks += static_cast<int>(goalPost.size());
  }
