...
bool Geometry::ballShouldBeVisibleInImage(const Vector2f &relBallPositionOnField, float ballRadius,
...
void Geometry::calculateAnglesForPoint(const Vector2f& point, const CameraMatrix& cameraMatrix,
...
bool Geometry::calculatePointByAngles(const Vector2f& angles, const CameraMatrix& cameraMatrix,
...
void Geometry::computeFieldOfViewInFieldCoordinates(const RobotPose& robotPose, const CameraMatrix& cameraMatrix,
    const CameraInfo& cameraInfo, const FieldDimensions& fieldDimensions, std::vector<Vector2f>& p)
{
  if(p.size() < 4)
    p.resize(4);
...
float Geometry::getDistanceBySize(const CameraInfo& cameraInfo, float sizeInReality,
...
bool Geometry::computeCircleOnFieldLevenbergMarquardt(
  const std::vector< Vector2f > &circlePoints, Geometry::Circle &circle)
{
  ...
  try
  {
    ...
    M << Mxx, Mxy, Mx,
        Mxy, Myy, My,
        Mx, My, static_cast<float>(number);
...
Vector2f Geometry::relative2FieldCoord(const Pose2f& rp, float x, float y) {
  return rp * Vector2f(x, y);
}

Vector2f Geometry::relative2FieldCoord(const Pose2f& rp,
                                        const Vector2f& relPosOnField) {
  return rp * relPosOnField;
}

Vector2f Geometry::fieldCoord2Relative(const Pose2f& robotPose,
                                        const Vector2f& fieldCoord) {
  ...
  return Vector2f(c * (fieldCoord.x() - x) - s * (fieldCoord.y() - y),
