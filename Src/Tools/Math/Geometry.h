...
/**
   ...
   */
  float angleBetween(const Vector2f& direction1, const Vector2f& direction2);
   ...
   */
  bool ballShouldBeVisibleInImage(const Vector2f &relBallPositionOnField, float ballRadius,
  ...
  bool computeCircleOnFieldLevenbergMarquardt(
   ...
   */
  Vector2f relative2FieldCoord(const Pose2f& rp, float x, float y);
   ...
   * @return Returns the ball positon in absolute coordinates
   */
  Vector2f relative2FieldCoord(const Pose2f& rp, const Vector2f& relPosOnField);
   ...
   */
  Vector2f fieldCoord2Relative(const Pose2f& robotPose, const Vector2f& fieldCoord);
