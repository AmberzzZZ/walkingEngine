...
void OwnSideModelProvider::update(OwnSideModel& ownSideModel)
{
  ...
  if(theGameInfo.secondaryState != STATE2_PENALTYSHOOT)
  {
    ...
    else if(theCognitionStateChanges.lastPenalty != PENALTY_NONE && theRobotInfo.penalty == PENALTY_NONE)
    {
      distanceWalkedAtKnownPosition = theOdometer.distanceWalked;
      largestXPossibleAtKnownPosition =theFieldDimensions.xPosOwnPenaltyMark; //-theFieldDimensions.centerCircleRadius - awayFromLineDistance;
...
//  std::cout<<"largestXPossible---->"<<ownSideModel.largestXPossible<<std::endl;
