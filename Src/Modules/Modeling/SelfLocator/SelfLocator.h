...
private:
  ...
  bool returnFromLeft = false;
  std::vector<Pose2f> penaltyPositions;
  unsigned lastTimeReset;

  std::chrono::high_resolution_clock::time_point lastTimeStampe;
  std::chrono::high_resolution_clock::time_point penalty_time;
  ...
  void HandleLowValidity(Pose2f robotpose);
