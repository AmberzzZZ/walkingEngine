...
{
    this->offsetColorAngle      = -0.5f;  //offset for approx. equal colorAngle detection rates for blue and red jerseys
    ...
    this->colorAngleBlue        =  2.5f;  //blue color angle
    this->colorAngleRed         =  0;    //red color angle
    this->pixelCalibrationRatio =  0.7f;  //how much calibration-pixels (in % of pixel sum)
    this->pixelSelectionRatio	=  0.15f; //how much color selection pixels (in % of pixel sum)
    this->topRegionHeight 		=  0.3f;  //vertical border for top area (in % of region-height)
    this->bottomRegionHeight 	=  0.5f;  //vertical border for bottom area (in % of region-height)
    this->gridSize=16;
...
float JerseyColorDetector::isBlueJersey(const Image& img, RobotRect rect){
    memset(histCbTop,0,sizeof(histCbTop));
    memset(histCrTop,0,sizeof(histCrTop));
    memset(histYBottom,0,sizeof(histYBottom));
    RobotRect topHalf;
...
int JerseyColorDetector::getAvgMaxValue(const Image& img, RobotRect rect, int thres, int ay, int ab, int ar, int by, int bb, int br){
    int cnt=0;
    int avgValue=0;
...
/**
     ...
     */
void JerseyColorDetector::setHistogramY(const Image& img, RobotRect rect, int* hist){
    int stepSize=max(1,(rect.xRight-rect.xLeft)/gridSize);
    for(int y=rect.yTop;y<=rect.yBottom;y+=stepSize){
        for(int x=rect.xLeft;x<=rect.xRight;x+=stepSize){
            int cy=getY(img,x,y);
 ...
 * @param hist array to write the histogram values
 */
void JerseyColorDetector::setHistogramCb(const Image& img, RobotRect rect, int* hist){
    int stepSize=max(1,(rect.xRight-rect.xLeft)/gridSize);
    for(int y=rect.yTop;y<=rect.yBottom;y+=stepSize){
        for(int x=rect.xLeft;x<=rect.xRight;x+=stepSize){
            int cb=getCb(img,x,y);
 ...
 * @param hist array to write the histogram values
 */
void JerseyColorDetector::setHistogramCr(const Image& img, RobotRect rect, int* hist){
    int stepSize=max(1,(rect.xRight-rect.xLeft)/gridSize);
    for(int y=rect.yTop;y<=rect.yBottom;y+=stepSize){
        for(int x=rect.xLeft;x<=rect.xRight;x+=stepSize){
            int cr=getCr(img,x,y);
...
/**
 * @brief reset pipeline to be ready for next detection
 */
void JerseyColorDetector::reset()
{
	memset(histCbTop,0,sizeof(histCbTop));
    memset(histCrTop,0,sizeof(histCrTop));
    memset(histYBottom,0,sizeof(histYBottom));
