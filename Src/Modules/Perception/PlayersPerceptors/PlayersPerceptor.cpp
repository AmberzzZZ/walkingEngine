...
bool PlayersPerceptor::scanJersey(PlayersPercept::Player& obstacle, float height, bool searchForPlayer)
{
  ...
  for(; x <= xEnd; x += step)
  {
    ...
    if (theECImage.colored[y][static_cast<int>(x)]==FieldColors::green)
    	continue;
    ...
    ColorModelConversions::fromYUVToRGB(cy,cb,cr,pr,pg,pb);
    int r = pr;
    int g = pg;
    int b = pb;
	
    //determine pixel color
    if ((cb-cr)>40 && cr>theFieldColor.seedCr && cy<(theFieldColor.seedY+20)) //maybe blue
    {
    ...
    else if (cr>(cb-10) && r-g>-10)
    {
    	if (theOwnTeamInfo.teamColor == TEAM_RED)
			teammate ++;
		else
			opponent ++;
    	RECTANGLE("module:PlayersPerceptor", x, y, x, y, 1, Drawings::solidPen, ColorRGBA::red);
    }
    else if (theECImage.colored[y][static_cast<int>(x)]==FieldColors::white)
    {
    	white ++;
...
//    }
     ...
     && std::abs(opponent - teammate) >= 1)
  {
    ...
    else if(white <= (opponent + teammate) * 3)
    {
      ...
      if (obstacle.ownTeam)
    	  DRAWTEXT("module:PlayersPerceptor", obstacle.realCenterX, y - 5, 20, ColorRGBA::yellow,"TEAMMATE");
//      else if (theOwnTeamInfo.teamColor == TEAM_RED)
//    	  DRAWTEXT("module:PlayersPerceptor", obstacle.realCenterX, y - 5, 10, ColorRGBA::yellow,"TEAM_RED");
      else
    	  DRAWTEXT("module:PlayersPerceptor", obstacle.realCenterX, y - 5, 20, ColorRGBA::yellow, "OPPONENT");
