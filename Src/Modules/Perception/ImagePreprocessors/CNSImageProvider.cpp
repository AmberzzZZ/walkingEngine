                                 ...
                                 __m128i imgL, __m128i img, __m128i imgR)
{
  ...
  sobelY = _mm_sub_epi16(blurX, currentIV.gaussIX);  // [+1 0 -1]^T*[1 2 1]*I
  gaussI = blur_epi16(blurX, previousIV.gaussIX, currentIV.gaussIX);  // [1 2 1]^T*[1 2 1]*I
...
void CNSImageProvider::update(CNSImage& cnsImage)
{
  ...
  cnsImage.setResolution(theECImage.saturated.width, theECImage.saturated.height);

  if(fullImage)
    cnsResponse(theECImage.saturated[0], theECImage.saturated.width,
                theECImage.saturated.height, theECImage.saturated.width,
                reinterpret_cast<short*>(cnsImage[0]), sqr(minContrast));
  else
    for(const Boundaryi& region : theCNSRegions.regions)
      cnsResponse(&theECImage.saturated[region.y.min][region.x.min], region.x.getSize(),
                  region.y.getSize(), theECImage.saturated.width,
