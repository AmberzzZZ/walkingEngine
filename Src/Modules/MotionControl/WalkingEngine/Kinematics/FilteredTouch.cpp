...
FilteredTouch::FilteredTouch(const float gyrXOffset,
		  const float gyrYOffset,
	      const float angleXOffset,
	      const float angleYOffset,
	      const float bodyPitchOffset)
   : init(true){
   ...
   imuOffset[GYR][0] =gyrXOffset;
   imuOffset[GYR][1] =gyrYOffset;
   targetOffset[GYR][0] = imuOffset[GYR][0];
   targetOffset[GYR][1] = imuOffset[GYR][1];
   targetOffset[ANG][0] = DEG2RAD(angleXOffset);
   targetOffset[ANG][1] = DEG2RAD(angleYOffset);
...
SensorValues FilteredTouch::getSensors(Kinematics &kinematics, SensorValues& RawSensor) {
   update = RawSensor;
   update.sensors[Sensors::InertialSensor_GyrX] *= 57.3f * 2.35f;
   update.sensors[Sensors::InertialSensor_GyrY] *= 57.3f * 2.35f;
   FilteredTouch::kinematics = kinematics;
//   std::cout<<"sensor00---->"<<update.sensors[Sensors::InertialSensor_AngleX]<<","<<update.sensors[Sensors::InertialSensor_AngleY]<<std::endl;
   if (init) 
   {
      init = false;
      state = update;
   ...
   else 
   {
      ...
      uint8_t i;
      for (i = 0; i < Sensors::numOfSensors; ++i){
...
//      std::cout<<"sensor11---->"<<update.sensors[Sensors::InertialSensor_AngleX]<<","<<update.sensors[Sensors::InertialSensor_AngleY]<<std::endl;
      filterSensors();
   }
