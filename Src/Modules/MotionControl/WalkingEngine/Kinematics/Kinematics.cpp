...
Kinematics::Kinematics() {
   std::vector<boost::numeric::ublas::matrix<float> > chest;
	parameters.bodyPitch = 0;
	parameters.cameraPitchBottom = 0;
	parameters.cameraPitchTop = 0;
	parameters.cameraRollBottom = 0;
	parameters.cameraRollTop = 0;
	parameters.cameraYawBottom = 0;
	parameters.cameraYawTop = 0;
   // Left side of body
...
void Kinematics::determineBodyExclusionArray(
   ...
   for (unsigned int part = 0; part < bodyParts.size(); part++) {
      ...
      for (unsigned int i = 0; i < bodyParts[part].size(); i++) {
         ...
         cIndex = std::min(std::max(cIndex, 0), (int) Pose::EXCLUSION_RESOLUTION);
         lIndex = std::min(std::max(lIndex, 0), (int) Pose::EXCLUSION_RESOLUTION);
         int index = lIndex;
