...
//    std::cout<<"odometryoffset---->"<<odometryOffset.rotation<<","<<odometryOffset.translation.x()<<","<<odometryOffset.translation.y()<<std::endl;
	if(theRobotInfo.bodyVersion >= RobotInfo::V5)
	{
		...
		Vector3f rotatedGyros = theInertialData.orientation * theInertialData.gyro.cast<float>();
		odometryOffset.rotation = theInertialData.gyro.z() * theFrameInfo.cycleTime;
	}
