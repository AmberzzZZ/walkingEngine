...
void KickEngineData::setParams(const MotionRequest &theMotionRequest, std::vector<KickEngineParameters>& params)
{
//	std::cout<<"params.size()---> "<<params.size()<<std::endl;
	for(auto& param : params)
	{
		if(!strcmp(param.name,"kickForward") && param.phaseParameters.size() == 5)
		{
      if(theMotionRequest.kickRequest.mirror)
        param.phaseParameters[0].duration = 1000;
      else
        param.phaseParameters[0].duration = 400;
      float kickDistance = theMotionRequest.kickRequest.kickDis;
      std::cout << "kickDistance: " << kickDistance << std::endl;
      if(kickDistance > 6000)
      {
        float fac1 = 1.f;
        param.phaseParameters[0].controlPoints[Phase::rightFootTra][1] = Vector3f(-20.f * fac1, -115.f,-200.f);
        param.phaseParameters[0].controlPoints[Phase::rightFootTra][2] = Vector3f(-20.f * fac1, -115.f,-200.f);
        param.phaseParameters[1].controlPoints[Phase::rightFootTra][1] = Vector3f(-60.f * fac1, -115.f,-170.f);
        param.phaseParameters[1].controlPoints[Phase::rightFootTra][2] = Vector3f(-60.f * fac1, -115.f,-170.f);
        param.phaseParameters[2].duration = 120;
      }
      else
      {
          float fac2 = 1.f;
          param.phaseParameters[0].controlPoints[Phase::rightFootTra][1] = Vector3f(-10.f * fac2, -115.f,-200.f);
          param.phaseParameters[0].controlPoints[Phase::rightFootTra][2] = Vector3f(-10.f * fac2, -115.f,-200.f);
          param.phaseParameters[1].controlPoints[Phase::rightFootTra][1] = Vector3f(-10.f * fac2, -115.f,-170.f);
          param.phaseParameters[1].controlPoints[Phase::rightFootTra][2] = Vector3f(-10.f * fac2, -115.f,-170.f);
          if(kickDistance > 5000)
              param.phaseParameters[2].duration = 160;    // 6000 - 5000
          else if(kickDistance > 4000)
              param.phaseParameters[2].duration = 185;    // 5000 - 4000
          else if(kickDistance > 3000)
              param.phaseParameters[2].duration = 190;    // 4000 - 3000
          else if(kickDistance > 2000)
              param.phaseParameters[2].duration = 200;    // 3000 - 2000
          else
              param.phaseParameters[2].duration = 210;    // < 2000
     }
		}
	}
}
