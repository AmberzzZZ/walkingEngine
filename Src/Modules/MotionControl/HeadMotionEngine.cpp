...
void HeadMotionEngine::update(HeadJointRequest& headJointRequest)
{
  ...
  float pan, tilt;
  if(theRobotInfo.bodyVersion >= RobotInfo::V5)
  {
    pan = requestedPan == JointAngles::off ? JointAngles::off : Rangef(theJointCalibration.joints[Joints::headYaw].minAngle, theJointCalibration.joints[Joints::headYaw].maxAngle).limit(requestedPan);
    tilt = requestedTilt == JointAngles::off ? JointAngles::off : Rangef(theJointCalibration.joints[Joints::headPitch].minAngle, theJointCalibration.joints[Joints::headPitch].maxAngle).limit(requestedTilt);
  }
  else
  {
    pan = requestedPan;
