...
void TeamDataSender::update(TeamDataSenderOutput& teamDataSenderOutput)
{
  if(theTeammateData.sendThisFrame)
  {
    ...
    // Role assign @tjark
    TEAM_OUTPUT(idRole, bin, RoleCompressed(theRole));
//    TEAM_OUTPUT(idRole, bin, theRole);

    switch(theRole.role)
    {
      case Role::keeper:
        TEAM_OUTPUT(idKeeperBehavior, bin, theRoleBehavior.keeperBehavior);
        break;
      case Role::striker:
        TEAM_OUTPUT(idStrikerBehavior, bin, theRoleBehavior.strikerBehavior);
        break;
      case Role::supporter:
        TEAM_OUTPUT(idSupporterBehavior, bin, theRoleBehavior.supporterBehavior);
        break;
      case Role::defender:
        TEAM_OUTPUT(idDefenderBehavior, bin, theRoleBehavior.defenderBehavior);
        break;
      case Role::stabber:
        TEAM_OUTPUT(idStabberBehavior, bin, theRoleBehavior.stabberBehavior);
        break;
      default:
        break;
    }
