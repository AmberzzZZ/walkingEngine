...
bool TeammateDataProvider::handleMessage(InMessage& message)
{
  switch(message.getMessageID())
  {
    ...
    case idTeammateFieldCoverage:
      ...
      case idKeeperKick:
        if(currentTeammate)
          message.bin >> currentTeammate->keeperKick;
        return true;
      case idDefenderKick:
        if(currentTeammate)
          message.bin >> currentTeammate->defenderKick;
        return true;
      case idStrikerKick:
        if(currentTeammate)
          message.bin >> currentTeammate->strikerKick;
        return true;
      case idSupporterKick:
        if(currentTeammate)
          message.bin >> currentTeammate->supporterKick;
        return true;
      case idStabberKick:
        if(currentTeammate)
          message.bin >> currentTeammate->stabberKick;
        return true;
      case idRole:
      if(currentTeammate)
      ...
      case idKeeperBehavior:
        if(currentTeammate)
          message.bin >> teammateDataPtr->roleBehavior.keeperBehavior;
        return true;
      case idStrikerBehavior:
        if(currentTeammate)
          message.bin >> teammateDataPtr->roleBehavior.strikerBehavior;
        return true;
      case idSupporterBehavior:
        if(currentTeammate)
          message.bin >> teammateDataPtr->roleBehavior.supporterBehavior;
        return true;
      case idDefenderBehavior:
        if(currentTeammate)
          message.bin >> teammateDataPtr->roleBehavior.defenderBehavior;
        return true;
      case idStabberBehavior:
        if(currentTeammate)
          message.bin >> teammateDataPtr->roleBehavior.stabberBehavior;
        return true;
    default:
      return true;
  }
...
void TeammateDataProvider::updateData(TeammateData& teammateData)
{
  ...
  // RoleBehavior @wzf
  if(!theRole.isKeeperOnline)
    teammateDataPtr->roleBehavior.keeperBehavior = KeeperBehavior();
  if(!theRole.isStrikerOnline)
    teammateDataPtr->roleBehavior.strikerBehavior = StrikerBehavior();
  if(!theRole.isSupporterOnline)
    teammateDataPtr->roleBehavior.supporterBehavior = SupporterBehavior();
  if(!theRole.isDefenderOnline)
    teammateDataPtr->roleBehavior.defenderBehavior = DefenderBehavior();
  if(!theRole.isStabberOnline)
    teammateDataPtr->roleBehavior.stabberBehavior = StabberBehavior();

  switch(theRole.role)
  {
    case Role::keeper:
      teammateDataPtr->roleBehavior.keeperBehavior = theRoleBehavior.keeperBehavior;
      break;
    case Role::striker:
      teammateDataPtr->roleBehavior.strikerBehavior = theRoleBehavior.strikerBehavior;
      break;
    case Role::supporter:
      teammateDataPtr->roleBehavior.supporterBehavior = theRoleBehavior.supporterBehavior;
      break;
    case Role::defender:
      teammateDataPtr->roleBehavior.defenderBehavior = theRoleBehavior.defenderBehavior;
      break;
    case Role::stabber:
      teammateDataPtr->roleBehavior.stabberBehavior = theRoleBehavior.stabberBehavior;
      break;
    default:
      break;
  }
