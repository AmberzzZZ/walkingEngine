...
void LEDHandler::setLeftEye(LEDRequest& ledRequest)
{
  ...
  else
  {
    ...
    bool teamBall = theFrameInfo.getTimeSince(theTeamBallModel.timeWhenLastValid) < 250;
...
void LEDHandler::setRightEye(LEDRequest& ledRequest)
{
  ...
  bool markSeen = theFrameInfo.getTimeSince(thePenaltyMarkPercept.timeLastSeen)<250;
  bool nearRobotSeen = false;//thePlayersPercept.isRobotNear;
  //no groundContact
  if(!theGroundContactState.contact/* && (theFrameInfo.time & 512)*/)
...
void LEDHandler::setHeadState(LEDRequest& ledRequest, LEDRequest::LED begin,
                              LEDRequest::LED end, LEDRequest::LEDState s)
{
  for (unsigned i = begin; i < end; ++i)
  {
    ledRequest.ledStates[i] = s;
  }
}
