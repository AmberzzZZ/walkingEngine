//WalkToTarget_SiQiang.h
//@SiQiang
//option(AlignMent,(Vector2f)(Vector2f(4500.f, 0.f)) target)
option(WalkToTarget_SiQiang, (const Pose2f&) speed, (const Pose2f&) pose)
{
	...
	if(pose != prePose)
	{
		...
		prePose = pose;
//		std::cout<<"pose.translation---------->"<<pose.translation.x()<<","<<pose.translation.y()<<std::endl;
...
//		std::cout<<"assignTarget---->"<<assignTarget.x()<<","<<assignTarget.y()<<std::endl;
	}
