option(DefenderOld)
{
	...
	common_transition
	{
		...
		target = defender.defenderDefencePosition();
...
//			else if(theTeamMateData.keeperKick[1] == true)
			else if(behavior.keeper.isControlBall == true)
			{
...
//      TEAM_OUTPUT(idDefenderKick, bin, false);
      behavior.defenderOutput.isControlBall = true;
			if(!ball.wasSeen() && ball.notSeenTime() > thePenaltyShootParameter.ballNotSeen)
				KeeperHeadControl(0, 3000, 3000, 0, false);
			else
				KeeperHeadControl(8000, 0, 3000, 0, false);
...
//			if(theTeamMateData.keeperKick[1] == true)
			if(behavior.keeper.isControlBall == true)
				goto walkInFrontOfGoal;
			else if(ball.suitable.x() > -1200 || (stopBall && ball.wasSeen() && ifSpreadNeeded && fabs(robot.rotation) < 1.4f))
				goto set;
...
//      TEAM_OUTPUT(idDefenderKick, bin, false);
      behavior.defenderOutput.isControlBall = false;
      WalkToPose(defencePositionInFrontOfGoal, Pose2f(3.f, 100.f, 100.f), true);
		}
	}
