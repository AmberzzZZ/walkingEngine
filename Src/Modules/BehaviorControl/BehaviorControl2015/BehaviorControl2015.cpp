...
#include "Options.h"
   public:
    ...
    void execute(const std::vector<OptionInfos::Option> &roots)
    {
      ...
      for (std::vector<Behavior::OptionInfos::Option>::const_iterator i = roots
          .begin(); i != roots.end(); ++i)
...
class BehaviorControl2015 : public BehaviorControl2015Base
{
  STREAMABLE(Parameters,
  {
    ...
    struct OptionInfos:Behavior::OptionInfos
    {
      typedef std::vector < Option > Options;
    }, ((OptionInfos) Options) roots,
    /**< All options that function as behavior roots. */
  });
  ...
  void update(BehaviorMotionRequest & motionRequest)
  {
	  DECLARE_DEBUG_DRAWING("module:Behavior2015:shootPath", "drawingOnField");
	  motionRequest = theMotionRequest;
  }

  /** Update the behavior status by copying it from the behavior */
  void update(BehaviorStatus & behaviorStatus)
  {
  ...
  void update(SPLStandardBehaviorStatus & splStandardBehaviorStatus)
  {
    splStandardBehaviorStatus = theSPLStandardBehaviorStatus;
  }

  void update(RoleBehavior & roleBehavior)
  {
 ...
 public:
  BehaviorControl2015()
      : behaviorData(const_cast<ActivationGraph &>(theActivationGraph),
                     theBehaviorLEDRequest, theBehaviorStatus,
                     theHeadMotionRequest, theArmMotionRequest,
                     theMotionRequest, theSPLStandardBehaviorStatus,
                     theRoleBehavior),
